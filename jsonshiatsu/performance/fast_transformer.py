...
class OptimizedJSONPreprocessor:
    ...
    _func_pattern = re.compile(r'^[a-zA-Z_][a-zA-Z0-9_.]*\s*\(\s*(.*)\s*\)\s*;?\s*$', re.DOTALL)
    _return_pattern = re.compile(r'^return\s+(.*?)\s*;?\s*$', re.DOTALL | re.IGNORECASE)
    _var_pattern = re.compile(r'^(?:const|let|var)\s+\w+\s*=\s*(.*?)\s*;?\s*$', re.DOTALL | re.IGNORECASE)
    ...
    def _detect_patterns(cls, text_preview: str) -> Tuple[bool, bool, bool, bool, bool]:
        ...
        has_special_quotes = any(char in text_preview for char in cls._quote_replacements)
        has_python_bools = 'True' in text_preview or 'False' in text_preview or 'None' in text_preview
        
        return has_markdown, has_comments, has_wrappers, has_special_quotes, has_python_bools
    ...
    def normalize_boolean_null(cls, text: str) -> str:
        ...
        if not any(keyword in text for keyword in ['True', 'False', 'None', 'yes', 'no', 'undefined']):
            return text
    ...
    def preprocess(cls, text: str, aggressive: bool = False, config=None) -> str:
        ...
        has_markdown, has_comments, has_wrappers, has_special_quotes, has_python_bools = cls._detect_patterns(text_preview)
