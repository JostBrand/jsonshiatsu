...
class OptimizedStreamingLexer:
    ...
    def read_string_stream(self, quote_char: str, validator: Optional[LimitValidator] = None) -> str:
...
class OptimizedStreamingParser:
    ...
    def parse_stream(self, stream: TextIO) -> Any:
        ...
        needs_preprocessing = (
            '```' in preview or '//' in preview or '/*' in preview or
            'return ' in preview or preview.count('"') != preview.count("'")
        )
    ...
    def _tokenize_stream_optimized(self, lexer: OptimizedStreamingLexer) -> Iterator[Token]:
    ...
    def current_token(self) -> Token:
        ...
        if self._current_token_pos != self.pos:
            if self.pos >= self.tokens_length:
                self._current_token_cache = self.tokens[-1] if self.tokens else Token(TokenType.EOF, "", Position(1, 1))
            else:
