...
class jsonshiatsuError(Exception):
    ...
    def _format_error(self) -> str:
        ...
        if self.position:
            lines.append(f"  at line {self.position.line}, column {self.position.column}")
        ...
        if self.context:
            ...
            if self.context.context_before or self.context.context_after:
                ...
                surrounding = f"{self.context.context_before}█{self.context.context_after}"
...
class ErrorReporter:
    ...
    def create_parse_error(self, 
                          message: str, 
                          position: Position,
                          suggestions: Optional[List[str]] = None) -> ParseError:
    ...
    def create_security_error(self,
                             message: str,
                             position: Optional[Position] = None,
                             suggestions: Optional[List[str]] = None) -> SecurityError:
...
class ErrorSuggestionEngine:
    ...
    @staticmethod
    def suggest_for_unexpected_token(token_value: str, expected: Optional[str] = None) -> List[str]:
