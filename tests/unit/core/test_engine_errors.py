...
import jsonshiatsu
from jsonshiatsu.core.engine import Parser, Lexer, _parse_internal, loads, load
from jsonshiatsu.utils.config import ParseConfig, ParseLimits
from jsonshiatsu.security.exceptions import ParseError, SecurityError, ErrorReporter
    ...
    def test_error_context_provision(self):
        ...
        try:
            ...
            self.assertTrue(any(keyword in error_message.lower() 
                              for keyword in ['expected', 'missing', 'invalid', 'could not', 'error']))
